...
impl InputState {
    ...
    pub fn update_mouse_button_state(&mut self, button: MouseButton, state: ElementState) {
...
impl Input {
    ...
    pub fn handle_keys(&mut self, event: KeyEvent) {
        if let PhysicalKey::Code(key) = event.physical_key {
            ...
            self.state.update_key_state(event);

            //Check if key has a bound action
            if let Some(action) = self.binds.key_map.get(&key) {

                match state {

                    ElementState::Pressed => {
                        if !self.actions.active.contains(action) {
                    ...
                    ElementState::Released => {
                        self.actions.active.remove(action);
    ...
    pub fn handle_mouse_button(&mut self, button: MouseButton, state: ElementState) {
        ...
        self.state.update_mouse_button_state(button, state);

        //Check if button has a bound action
        if let Some(action) = self.binds.mouse_map.get(&button) {
    ...
    //clears frame
    //should only be done at end of engine loop
    pub fn end_frame(&mut self) {

        //Clear one-frame action states
        self.actions.just_pressed.clear();
        self.actions.just_released.clear();

        //clear one-frame raw input states
